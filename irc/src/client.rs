...
impl<'a> IrcClientBuilder<'a> {
	...
	pub fn connect(self) -> Result<(IrcClientReader, Addr<IrcClientWriter>), IrcError> {
		let (mut reader, writer) = connect(self.url)?;
...
pub fn connect(url: &str) -> Result<(IrcClientReader, Addr<IrcClientWriter>), IrcError> {
	...
	let reader = IrcClientReader {
		reader: BufReader::new(stream.try_clone().map_err(|e| IrcError::ConnectionFailed("Cloning stream".to_owned()))?)
	};

	let writer = Arbiter::start(|ctx| IrcClientWriter{
...
impl IrcClientReader {
	...
	pub fn wait_for_line(&mut self) -> Result<String, IrcError> {
		...
		self.reader.read_line(&mut line).map_err(|e| IrcError::ReadFailed(format!("{:?}", e)))?;
...
impl IrcClientWriter {
	...
	pub fn send_line(&mut self, line: &str) -> Result<(), IrcError> {
		let _ = self.writer.write(line.as_bytes()).map_err(|e| IrcError::WriteFailed(format!("{:?}", e)))?;
		self.send_new_line()?;
		self.writer.flush();
		Ok(())
	}
...
impl Handler<SendLine> for IrcClientWriter {
	...
	fn handle(&mut self, msg: SendLine, ctx: &mut Self::Context) -> <Self as Handler<SendLine>>::Result {
...
impl Handler<JoinChannel> for IrcClientWriter {
	...
	fn handle(&mut self, msg: JoinChannel, ctx: &mut Self::Context) -> <Self as Handler<JoinChannel>>::Result {
		self.join(&msg.0)
	}
}
...
impl Handler<SendChannelMessage> for IrcClientWriter {
	...
	fn handle(&mut self, msg: SendChannelMessage, ctx: &mut Self::Context) -> <Self as Handler<SendChannelMessage>>::Result {
